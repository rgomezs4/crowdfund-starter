...
class CreateCampaign extends React.Component {
  ...
  async handleSubmit(e) {
    if (this.state.goal <= 0) {
      ...
      await this.createCampaign(this.state.name, this.state.beneficiary, this.state.goal)
  ...
  async createCampaign(name, beneficiary, goal, duration) {
    await newCampaign(name, beneficiary, goal, duration);
  }
...
const newCampaign = async (name, beneficiary, goal, duration) => {
  const Dashboard = window.web3.eth.contract(ABI);
  let dashboardInstance = Dashboard.at(ADDRESS);
  return new Promise((resolve, reject) => {
    dashboardInstance.newCampaign(name, beneficiary, goal, duration, null, (err, data) => {
      if (err) {
        reject(err)
      } else {
