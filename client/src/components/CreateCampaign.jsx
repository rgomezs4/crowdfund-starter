...
import { ABI_DASHBOARD, ADDRESS } from '../constants/constants'
...
class CreateCampaign extends React.Component {
  ...
  async handleSubmit(e) {
    if (this.state.goal <= 0) {
      ...
      await this.createCampaign(this.state.name, this.state.beneficiary, this.state.goal, this.state.duration)
  ...
  async createCampaign(name, beneficiary, goal, duration) {
    ...
    return new Promise((resolve, reject) => {
      dashboardInstance.newCampaign(name, beneficiary, goal, duration, (err, data) => {
        if (err) {
          reject(err)
        } else {
